...
class AuthTest(TestCase):
    ...
    def test_no_auth_case_returns_401(self):
        response = self.client.post("/case", json={"subject": "Test", "description": "Test"})
        self.assertEqual(response.status_code, 401)
...
class APIKeyTest(TestCase):
    ...
    def test_authenticate_expired_key(self):
        api_key, raw_key = APIKey.create_key(name="expired-key", expires_at=timezone.now() - timezone.timedelta(days=1))
        result = APIKey.authenticate(raw_key)
