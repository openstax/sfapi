...
class AccountTest(TestCase):
    ...
    def test_account_create(self):
        account = Account.objects.create(
            name="Test Account", type="Customer - Direct", city="Houston", state="TX", country="USA"
        )
...
class BulkSyncTest(TestCase):
    ...
    def test_bulk_create_accounts(self):
        accounts = [self._make_mock_account(f"001{i:015d}", f"School {i}") for i in range(10)]
    ...
    def test_bulk_update_accounts(self):
        # First create them
        accounts = [self._make_mock_account(f"001{i:015d}", f"School {i}") for i in range(5)]
    ...
    def test_bulk_mixed_create_update(self):
        # Create 3 accounts
        accounts = [self._make_mock_account(f"001{i:015d}", f"School {i}") for i in range(3)]
        update_or_create_accounts(accounts)

        # Add 2 new + update 3 existing (upsert returns total count)
        accounts.extend([self._make_mock_account(f"001{i:015d}", f"School {i}") for i in range(3, 5)])
        count = update_or_create_accounts(accounts)
        self.assertEqual(count, 5)
